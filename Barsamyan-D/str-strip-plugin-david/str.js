...
function createLeftColumn(section) {
    ...
    questionBtn.addEventListener("click", () => instructionsDropdown.classList.toggle("active"));
...
function createRightColumn(section, userInputs, model) {
    ...
    if (section.max_length) {
        const { counter, feedback } = createTextAreaWithCounter(textarea, section, userInputs, model);
        rightCol.append(textarea, counter, feedback);
    } else {
        textarea.addEventListener("input", createInputHandler(section, userInputs, model));
